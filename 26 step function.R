...
data_gen = function(N = 1000, type = "step"){
  if(type == "step"){
    ...
    n1 = 0
    t = seq(1,N,1)/N
    data = K/(1+exp(-(gamma*t+n0))) + n1 + rnorm(N)
  }
  return(data)
}
...
data = data_gen(1000, "sigmoid")
ggplot() + geom_line(aes(x=(1:N)/N, y=data))


fit1 = vb(stan_model("step3.stan"), data = list(d=data, N=N, t=(1:N)/N), algorithm="meanfield")
