...
data_gen = function(N = 1000, type = "step"){
  if(type == "step"){
    tau = 400
    data = rep(0, N)
    data[400:N] = data[400:N] + 1.5
    data = rpois(N, exp(data))
  }else{
    K = 4
    gamma = 30
    n0 = -15
    n1 = 0.1
    t = seq(1,N,1)/N
    data = rpois(N, K/(1+exp(-(gamma*t+n0))) + n1)
  }
  return(data)
}
...
data = data_gen(1000, "ste")
ggplot() + geom_line(aes(x=(1:N)/N, y=data))


fit1 = vb(stan_model("step3 count.stan"), data = list(d=data, N=N, t=(1:N)/N), algorithm="meanfield")
...
#---------------------------------------------------------------------------------------
# sigmoidと比較する
fit2 = vb(stan_model("logistic.stan"), data = list(d=data, N=N, t=1:N), algorithm = "fullrank")
fit2 = stan(file = "logistic.stan", data = list(d=data, N=N, t=(1:N)/N),
           iter=100000, warmup = 90000, chain=4)
